...
/**
 * Create a tutorial progress store persisted to `storage` under STORAGE_KEY.
 * The app uses the `tutorials` singleton below; the factory exists for tests.
 */
export function createTutorials(storage: TutorialStorage | undefined = browserStorage()) {
	let initial: TutorialState = {};
	try {
		initial = parseTutorialState(storage?.getItem(STORAGE_KEY));
	} catch {
		initial = {};
	}

	const store = writable<TutorialState>(initial);
