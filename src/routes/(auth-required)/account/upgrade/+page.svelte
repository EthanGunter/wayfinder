<script lang="ts">
	...
	async function checkAccountIssues() {
		...
		try {
			...
			// For now, simulate checking for common issues
			const user = await userPromise;
			const issues = [];
			
			if (user && isLocalUser(user)) {
				if (!user.email) {
					issues.push('No email address for account recovery');
				}
	...
	function isLocalUser(user: any): user is LocalUserProxy {
		return user && 'isSynced' in user;
	}
...
<div class="upgrade-page">
	...
	{:then user}
		{#if user && isLocalUser(user) && !user.isSynced}
			<div class="upgrade-container">
				...
				{:else if accountIssues.length > 0}
					<div class="issues-section">
						...
						<form class="resolve-form" onsubmit={(e) => { e.preventDefault(); handleResolveIssues(); }}>
				...
				{:else if !isMigrating}
					<div class="ready-section">
						...
						<div class="action-buttons">
							<button class="btn-upgrade" onclick={handleMigration}>
								Start Upgrade
							</button>
							<button class="btn-secondary" onclick={() => goto('/account')}>
								Maybe Later
							</button>
						</div>
					</div>
